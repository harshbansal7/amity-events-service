...
def init_auth_routes(mongo):
    ...
    def register():
        ...
        return (
            jsonify({"message": "Registration successful", "user_id": str(user_id)}),
            201,
        )
    ...
    def login():
        ...
        if not data or not data.get("enrollment_number") or not data.get("password"):
            return (
                jsonify({"error": "Enrollment number and password are required"}),
                400,
            )

        # Check if it's an external participant (enrollment starts with 'EXT')
        if data["enrollment_number"].startswith("EXT"):
            user = external_participant_model.get_by_temp_enrollment(
    ...
    def verify_event_code():
        ...
        return (
            jsonify({"message": "Valid event code", "event_name": event["name"]}),
            200,
        )
    ...
    def register_external():
        ...
        # Validate event code
        event = mongo.db.events.find_one(
            {"event_code": data.get("event_code"), "allow_external": True}
        )
        ...
        (
            temp_enrollment,
            temp_password,
        ) = external_participant_model.generate_temp_credentials()
        ...
        _ = external_participant_model.create_external_participant(
            participant_data, event["event_code"], password_hash
        )

        # Send credentials via email
        credentials = {"enrollment_number": temp_enrollment, "password": temp_password}

        from app.utils.mail import MailgunMailer

        mailer = MailgunMailer()
        mailer.send_external_credentials(
            data["email"], data["name"], event["name"], credentials
        )
